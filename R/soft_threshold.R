...
soft_thresholded_normal_variance <- function(lambda, mu, sigma) {
  2 * mu^2 + 2 * lambda^2 + 2 * sigma^2 -
    ...
    ((mu - lambda)^2 + sigma^2) * (pnorm(lambda, mu, sigma)) + (mu - lambda) * sigma^2 * dnorm(lambda, mu, sigma) - soft_thresholded_normal_mean(lambda, mu, sigma)^2
}

soft_thresholded_mse <- Vectorize(function(lambda, mu, sigma) {
  return(sum((mu - soft_thresholded_normal_mean(lambda, mu, sigma))^2) + sum(soft_thresholded_normal_variance(lambda, mu, sigma)))
}, vectorize.args = "lambda")

soft_thresholded_optimal_lambda <- function(mu, sigma, lambda_max = 2) {
  #optimize(\(lambda) soft_thresholded_mse(lambda, mu, sigma), c(0, lambda_max))$minimum
  lambdas <- seq(0, lambda_max, length.out = 1e3)
  mses <- soft_thresholded_mse(lambdas, mu, sigma)
